...
Load WiLoR ─model─► Hand: WiLoR ──(hands_from)──► Body: NLF               │
                                                                         ▼
...
| **Body: NLF** | Robust single-image body → SMPL-X (neutral shape, flat hands). Optional `hands_from`. |
| **Full Body: Multi-HMR** | One-pass expressive whole-body SMPL-X (body + hands + face). |
| **Hand: WiLoR** | In-the-wild hand reconstruction → SMPL-X hand pose (feeds `hands_from`). |
...
| **MANO** *(for WiLoR)* | `MANO_LEFT.pkl`, `MANO_RIGHT.pkl` | WiLoR `mano_data/` | MPI — **registration** | [mano.is.tue.mpg.de](https://mano.is.tue.mpg.de/) |
...
3. *(Optional, for sharp hands with NLF)* **Load WiLoR → Hand: WiLoR**, and wire its output into **Body: NLF**'s `hands_from`.
...
- A CUDA GPU is recommended; **NLF is GPU-only** (its TorchScript bakes `cuda:0`). Multi-HMR and WiLoR fall back to CPU automatically on OOM.
